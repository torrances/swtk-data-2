...
public class GenerateNounFrameControllers {

	public static LogManager logger = new LogManager(GenerateNounFrameControllers.class);

	public static Set<String> idSet = new TreeSet<String>();

	public static String JAVA_ADD_PATTERN = "\tif (\"%s\".equals(key)) return WordnetNounFrame%s.get(ID);\n";

	public static File TEMPLATE = WordnetMdaConstants.TemplateNounFrameController;

	public static File SOURCE = WordnetMdaConstants.SrcFileDataNoun;
	...
	public static void process() throws BusinessException {
		...
		for (String line : FileUtils.toList(SOURCE, Codepage.UTF_8)) {
			...
			idSet.add(bean.getId().substring(0, 4));
		}
		...
		String outFileName = WordnetMdaConstants.OutFileFrameController;
		String outPkgName = WordnetMdaConstants.OutPathNounFrameController;
